...
export class HandoffService {
  static createHandoffPackage(
    ...
    if (sessions.length === 0) {
      throw new Error(
        "Handoff readiness gate failed: No approved discovery session exists.",
      );
    }
    ...
    if (baselines.length === 0) {
      throw new Error(
        "Handoff readiness gate failed: No frozen requirement baseline exists.",
      );
    }
    ...
    const basePackageData = {
      ...
      stakeholders:
        stakeholders.length > 0
          ? stakeholders
          : [{ name: "Founder / Lead", role: "Sponsor" }],
      problemStatement:
        opp.score.explanation ||
        "Client needs automated workflow and discovery platform implementation.",
      currentState:
        currentState.length > 0
          ? currentState
          : [{ processName: "Manual Operations", painPoint: "High effort" }],
      requirementBaseline: {
      ...
      constraints:
        latestBaseline.constraints.length > 0
          ? latestBaseline.constraints
          : [{ description: "Budget cap CAD 150/mo", category: "financial" }],
      risksAndAssumptions:
        latestBaseline.risksAndAssumptions.length > 0
          ? latestBaseline.risksAndAssumptions
          : [
              {
                description: "API access granted by client",
                type: "assumption" as const,
                impact: "high",
              },
            ],
      successMeasures:
        latestBaseline.successMeasures.length > 0
          ? latestBaseline.successMeasures
          : [{ metric: "Pipeline velocity", target: "2x improvement" }],
      commercialScope,
      supportingArtifacts,
      openItems,
    ...
    const manifestChecksum = sha256CanonicalJson(basePackageData);
  ...
  static async deliverPackageToDeliveryTool(
    ...
    const outbox = Array.from(db.outboxEvents.values()).find(
      (e) =>
        e.aggregateId === packageId &&
        e.eventType === "handoff.release.requested",
    );
    ...
    try {
      ...
      if (response.ok && resData.status === "accepted") {
        ...
        if (outbox) {
          ...
          outbox.attempts += 1;
          outbox.lastAttemptAt = new Date().toISOString();
          outbox.errorMessage = JSON.stringify(
            resData.rejectionReasons ||
              resData.error ||
              "Delivery tool rejected package",
          );
        }

        return {
          success: false,
          rejectionReasons: resData.rejectionReasons || resData.error,
        };
      }
    } catch (err: any) {
      if (outbox) {
      ...
      return {
        success: false,
        rejectionReasons: { code: "NETWORK_ERROR", message: err.message },
      };
    }
  }
}
